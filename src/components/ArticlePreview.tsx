...
export default function ArticlePreview({
  ...
  return (
    <article className={`relative flex flex-col ${className}`}>
      <h3 className="mt-[0.3rem] mb-[1.5rem] text-[24px] leading-[1.4] font-pixel">
        <a
          href={`/article/${slug}`}
      ...
      <figure className="relative order-[-1] mb-[2rem] pb-[100%] rounded-[0.6rem] overflow-hidden bg-gray-100">
        <a href={`/article/${slug}`} title={imageAlt}>
          <img alt={imageAlt} src={image} loading="lazy" className="absolute w-full h-full rounded-[0.6rem] object-cover" />
        </a>
      </figure>
